...
def is_NFC_grammar(g):
    for (nt, rules) in g.items():
        for rule in rules:
            if not is_NFC_rule(nt, rule):
                return False
    return True
...
def test_to_NFC1():
    ...
    g.grammar = {'S': [['a', 'A']], 'X': [['a', 'Y'], ['eps'], ['b', 'Y']], 'Y': [['a', 'Y'], ['b', 'Y'], ['c', 'c']],
                 'B': [['eps'], ['y', 'X'], ['y']], 'A': [['X', 'B'], ['y', 'X'], ['y']]}
    assert not is_NFC_grammar(g.grammar)
    g.to_NFC()
    assert is_NFC_grammar(g.grammar)
